...
export async function POST(req: Request) {
  try {
    ...
    // --- 🛡️ BUG FIX: จัดการ Action ซ้ำซ้อน ---
    if (prompt === gameState.last_action) {
      // ถ้าผู้เล่นทำ Action เดิมซ้ำ ให้ใช้ Fallback ที่มีความหมาย
      ...
      // --- 🖋️ Prompt V5: เน้นความกระชับและเป็นธรรมชาติ ---
      const systemPrompt = `
คุณคือ Game Master เล่าเรื่องราวของ AI LifeSim ในรูปแบบนิยายสั้นภาษาไทย
ห้ามพิมพ์วันที่หรือเวลาใด ๆ ในการตอบเด็ดขาด
...
**รูปแบบการตอบ**
1. เล่าเหตุการณ์ใหม่ 1 ช็อต แบบภาพยนตร์สั้น (ย่อหน้าเดียว)
...
`;
    ...
    // --- ส่วนคำนวณและบันทึกข้อมูล (เหมือนเดิม) ---
